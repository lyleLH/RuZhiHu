...
//            retVal = [NSString stringWithFormat:@"%d%@", distance, (distance == 1) ? @"秒前" : @"秒前"];
            ...
            retVal = [NSString stringWithFormat:@"%d%@", distance, @"分钟前"];
        }else if (distance < 60 * 60 * 24) {
            distance = distance / 60 / 60;
            retVal = [NSString stringWithFormat:@"%d%@", distance, @"小时前"];
        }
...
- (NSDate *)dateAfterSecond:(int)second{
    return self;
//    NSDate *date = [self dateByAddingTimeInterval:<#(NSTimeInterval)#>]
}
