...
+ (UIImage*)blackWhite:(UIImage*)inImage
{
	...
	NSInteger w = CGImageGetWidth(inImageRef);
	NSInteger h = CGImageGetHeight(inImageRef);
	...
	for(NSInteger y = 0;y< h;y++)
	{
		...
		for (NSInteger x = 0; x<w; x++)
		{
			...
			NSInteger red = (unsigned char)imgPixel[pixOff];
			NSInteger green = (unsigned char)imgPixel[pixOff+1];
			NSInteger blue = (unsigned char)imgPixel[pixOff+2];
			
			NSInteger bw = (int)((red+green+blue)/3.0);
...
+ (UIImage*)cartoon:(UIImage*)inImage
{
	...
	for(NSInteger y = 0;y< h;y++)
	{
		...
		for (NSInteger x = 0; x<w; x++)
		{
			...
			NSInteger ava = ((red+green+blue)/3.0);
			
			NSInteger newAva = ava>128 ? 255 : 0;
...
+(UIImage*)scanLine:(UIImage*)inImage
{
	...
	for(NSInteger y = 0;y< h;y+=2)
	{
		...
		for (NSInteger x = 0; x<w; x++)
		{
			...
			NSInteger newR,newG,newB;
			NSInteger rr = red *2;
			newR = rr > 255 ? 255 : rr;
			NSInteger gg = green *2;
			newG = gg > 255 ? 255 : gg;
			NSInteger bb = blue *2;
			newB = bb > 255 ? 255 : bb;
...
- (UIImage *)scaledToPixels:(CGFloat)pixels{
    ...
    if (size.width > pixels || size.height > pixels) {
        if (size.width > size.height) {
            size.height = size.height / (size.width / pixels);
            size.width = pixels;
        }else{
            size.width = size.width / (size.height / pixels);
            size.height = pixels;
        }
    }
    
    UIImage *image = [UIImage imageWithImage:[UIImage fixOrientation:self] scaledToSize:size];
